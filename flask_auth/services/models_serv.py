...
class UserServ(User):
    ...
    def add_or_del_role_user_billing(cls, json: dict, add: bool = False) -> tuple[Response, HTTP]:
        '''A method specifically for billing. The method adds or removes the user's role.
        Adds all keys to the blocklist, in both cases. The output is a ready Response.'''
        try:
            role = RoleServ.get_obj_by_role(json['role'])
